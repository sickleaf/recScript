...
gdrivearchivedir="$mntdirectory/gdrive/radio_archive"
...
rtmpdump \
         ...
         --stop ${DURATION} \
         --hashes \
         --flv "/tmp/${date}_${PREFIX}"
#         --flv "~/Radio/${date}_${PREFIX}"

sudo /usr/src/FFmpeg/ffmpeg -loglevel fatal -y -i "/tmp/${date}_${PREFIX}" -acodec libmp3lame -ab 64k "${outdir}/${date}_${PREFIX}.mp3"
if [ $? = 0 ]; then
  rm -f "/tmp/${date}_${PREFIX}"
fi

sudo mv ${outdir}/${date}_${PREFIX}.mp3 ${basedir}

sudo /usr/bin/mp3splt -s -p min=$spltmin,th=$spltth -d "${basedir}/${date}_${PREFIX}" "${basedir}/${date}_${PREFIX}.mp3"
